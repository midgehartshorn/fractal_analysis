...
#img_data = fits.getdata('D111.fits')
img_data = fits.getdata('D112.fits') # 
#img_data = fits.getdata('D33.fits')
...
# size of grid box using method from Liu et al 2014 (s|M)
...
s = np.arange(2,m/2, dtype='int') #size of grid box using most common method
s
...
def countSingle(img_data,x,y,h,s,k):
    ...
    box_count += (np.ceil(loc_max/h)-np.ceil(loc_min/h)+1)
    return box_count
...
(N_total, scales) = countAll(img_data,s)
...
plt.plot(np.log(scales),np.log(N_total), '.', mfc='none')
plt.plot(np.log(scales), np.polyval(fit, np.log(scales)))
plt.xlabel('$\log 1/r$')
plt.ylabel('$\log N_r$')
plt.show()
#print(fit) # the fractal dimension is the fitting coefficient
